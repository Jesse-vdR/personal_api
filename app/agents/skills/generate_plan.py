...
Input: {"week_start": "YYYY-MM-DD", "context": "free-form notes"}.
`week_start` defaults to next Monday in user TZ. `context` is optional
free-form prose that gets injected into the user message under an
'Extra context from user' section. Reads per-user profile, long-term
plan, tracks, goals, and the last 4 weeks of training_events. Calls
Claude Opus 4.7 via a forced tool call (`submit_plan`) so the response
...
(goals, recent events, week_start, user context) live in the user
message after the breakpoint so the cache holds across runs.
"""
...
_SUBMIT_PLAN_TOOL: dict[str, Any] = {
    ...
    "description": (
        ...
        "`week_start` (YYYY-MM-DD, must equal the requested Monday); "
        "`days`, an array of exactly 7 day objects in chronological order "
        "Mon..Sun, each with `date` (YYYY-MM-DD) and `exercises`, an "
        "ordered list of exercises with slug, display, unit "
        "('reps' | 'duration_s' | 'walks'), target_total, per_set, sets "
        "(target_total must equal per_set * sets); and `rationale`, a "
        "short plain-prose explanation (~2-3 sentences, max ~400 chars) "
        "of the main decisions you made — volume distribution, what you "
        "scaled up or down, how you accommodated the user's extra "
        "context. The runner reshapes the days array into "
        "training/plan.json's date-keyed map before storage."
    ),
    "strict": True,
    "input_schema": {
        "type": "object",
        "properties": {
            ...
            "rationale": {
                "type": "string",
                "description": (
                    "Plain-prose summary of the main decisions — volume "
                    "distribution, scale-up/scale-down, accommodation of "
                    "user's extra context. ~2-3 sentences, ~400 chars."
                ),
            },
        },
        "required": ["week_start", "days", "rationale"],
        "additionalProperties": False,
    },
}
...
_SYSTEM_INTRO = (
    ...
    "stages. If the user supplies extra context for this week, treat it "
    "as authoritative (e.g. injuries, travel, energy). Output via the "
    "submit_plan tool; do not reply in prose."
)


def resolve_week_start(raw: Any) -> date:
    if raw:
        parsed = date.fromisoformat(str(raw))
        if parsed.weekday() != 0:
            raise ValueError(
                f"week_start must be a Monday, got {parsed} "
                f"({parsed.strftime('%A')})"
            )
        return parsed
...
def collect_inputs(
    ...
    """JSON-serialisable bundle of everything generate_plan reads.

    Used by the skill at run-time and exposed verbatim via the preview
    endpoint so the UI can show exactly what the agent will see.
    """
    ...
    return {
        ...
        "long_term": long_term.body if long_term else None,
        "tracks": [
...
def run(
    ...
    *,
    ...
    week_start = resolve_week_start(input.get("week_start"))
    context = (input.get("context") or "").strip() or None

    inputs = collect_inputs(session, user, week_start)

    system_blocks = _build_system_blocks(inputs)
    user_message = _build_user_message(inputs, context)
...
def _recent_events(
    session: Session, user_id: int, week_start: date
) -> list[dict[str, Any]]:
...
def _build_system_blocks(inputs: dict[str, Any]) -> list[dict[str, Any]]:
    sections: list[str] = [_SYSTEM_INTRO, ""]

    sections.append("## User profile")
    sections.append(_markdown_or_json(inputs["profile"]))
    sections.append("")

    sections.append("## Long-term plan")
    sections.append(_markdown_or_json(inputs["long_term"]))
    sections.append("")

    sections.append("## Tracks (progression ladders)")
    sections.append(json.dumps(inputs["tracks"], sort_keys=True, indent=2))

    text = "\n".join(sections)
    return [{"type": "text", "text": text, "cache_control": {"type": "ephemeral"}}]


def _build_user_message(inputs: dict[str, Any], context: str | None) -> str:
    sections: list[str] = []
    sections.append(
        f"Generate the weekly plan for week_start = {inputs['week_start']} (Monday)."
    )
    sections.append("")
    sections.append("## Active goals")
    sections.append(json.dumps(inputs["goals"], sort_keys=True, indent=2))
    sections.append("")
    sections.append("## Training events — last 4 weeks")
    sections.append(json.dumps(inputs["events"], sort_keys=True, indent=2))
    sections.append("")
    if context:
        sections.append("## Extra context from user")
        sections.append(context)
        sections.append("")
    sections.append("Now call submit_plan with the full Mon..Sun schedule plus a rationale.")
    return "\n".join(sections)


def _markdown_or_json(body: Any) -> str:
    if body is None:
